...
class TmdbSearch
 ...
 def find_movie_basic(id)
   @_basic = JSON.parse(service.get_url.get("/3/movie/#{id}").body, symbolize_names: true)
 end

 def find_movie_cast(id)
   @_cast = JSON.parse(service.get_url.get("/3/movie/#{id}/credits").body, symbolize_names: true)[:cast].first(10)
 end
