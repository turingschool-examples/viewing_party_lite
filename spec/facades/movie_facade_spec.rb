...
RSpec.describe MovieFacade do 
  ...
  it 'can return movies matching a search keyword' do
    json_response = File.read('spec/fixtures/star_trek_search.json')
    stub_request(:get, "https://api.themoviedb.org/3/search/movie?api_key=73c37a0fb30ac13df0c132c1df149b3f&query=Star%20Trek:%20First%20Contact").
         with(
           headers: {
       	  'Accept'=>'*/*',
       	  'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3',
         ...
         to_return(status: 200, body: json_response, headers: {})
    movies = MovieFacade.movie_search("Star Trek: First Contact")
    expect(movies).to be_a(Array)
    expect(movies).to be_all Movie
